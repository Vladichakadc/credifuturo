...
- **Profit distribution (reparto de utilidades).** The fund's profit is divided in proportion to each member's **capital weighted by the months it works**: the method savings institutions use. `services/reparto.js` holds the arithmetic (pure, no DB), `GET /savings/ranking` applies it, and `client/src/pages/shared/RepartoUtilidadesPage.jsx` renders it. The screen replaced the old "Ranking de Ahorro" and its Top-3 podium.
      ...
      capitalPonderado = Σ ( importe × días que trabajará ÷ días del año )

  1 January weighs 100%, 1 July ~50%, 31 December ~0%. It is a **weighted sum, not an average** — every line can be read and checked on its own, which is what defending a distribution in a members' meeting requires.
  - **The weight is per day, not per month.** Weighting by month made 1 July and 30 July identical, and that is unfair in a way the fund can measure: the money sits in the NU account from the day it arrives and NU settles interest daily, so the member who deposited on the 1st gave the fund twenty-nine more days of work than the one who deposited on the 30th. On $1.000.000 that is **$79.452** of weighted capital the punctual member was handing over. `pesoDeFecha(fecha, periodo) = díasInclusive(fecha, 31-dic) ÷ periodo.dias`. The day of entry counts, so 31 December is `1/365` rather than a flat zero — a day of work is a day. `periodo.dias` is computed, never assumed: a leap year is 366 and dividing it by 365 would push 1 January above 1.
  - **A month row no longer has a single weight.** `porMes[i].peso` is the *effective* weight — `ponderado ÷ movido` — because two deposits in the same month now weigh differently. When the month's movements cancel out exactly, it falls back to the reference weight of the month's first day. `pesoDeMes(mes, periodo)` exists only for labels and legends.
  - **The period's two ends both travel to the client.** `inicio` and **`fin`** are both in the payload: without `fin` the member's simulator cannot compute the weight of a date and silently returned 0% for every day, including 1 January.
  ...
  - **In the running year the weights still run to 31 December**, so the figures are a projection to year-end, and the screen says so rather than presenting them as final.
  ...
  - **One landing, no figure said twice.** The route hero already names the page, so the green header carries no prose of its own — just the year selector, the role link and **three** figures that appear nowhere else on the screen: what the fund earned, **what that yielded** (`ganancia ÷ capital ponderado`, the same rate the admin panel's "Retorno del Capital" now shows), and how many members it splits between. `Capital ponderado` left the header because it is the denominator of the rate right beside it, and the cut date moved into the running-year notice, which was already saying it. The chart section below no longer repeats the profit, and the member's own yield moved into "Tu parte", where a personal figure belongs.
  - **The distribution chart opens like a fund statement.** What used to be there was a single bar with one segment per member — a fine picture of "one pot being divided", but a member got nothing from it: twenty-five unlabelled segments, and finding your own meant hovering one by one. `GraficoReparto` now leads with the number every fund statement leads with — **the yield**: `ganancia ÷ capital ponderado` is what the capital that actually worked returned, and each member's own `su utilidad ÷ lo que ahorró` is the one that *varies* with when their money came in. That difference is the whole lesson of the method and it appeared nowhere before. Below it, one horizontal bar per member (long Spanish names make the vertical layout unreadable), sorted, the member's own bar in gold, and **a reference line at the average** — without it a member sees their figure but cannot tell whether it is a lot or a little. In the admin view `yoId` is not passed: a personal figure has no place on the screen the distribution is governed from, so the third KPI becomes the average share instead.
  - **The table shows what the member recognises, not the intermediate arithmetic.** `Ahorro → Peso → % de la ganancia → Le corresponde`. `capitalPonderado` was removed from it: it is the working figure, not the result, and it still appears in the per-member breakdown that opens underneath, which is where you check where it comes from. The percentage column says explicitly that it is the share **of the fund's total profit**.
  ...
  Regression suites: `node server/pruebas_reparto.js` (118 assertions, pure arithmetic) and `node server/pruebas_reparto_http.js` (66 over the real route, including that `/dashboard-stats` and `/savings/ranking` agree on the weighted capital).
...
- **"Retorno del Capital" contradicted the table three inches below it.** The KPI summed its own `totalInteresesPagados + rentabilidadCajaNU + totalPenaltyValue`, while the "Ganancia total del fondo" row in the same component uses `gananciaRealYtd`. They are not the same figure: `totalPenaltyValue` **excludes** the "Descuento Total Anual Penalizacion", which in 2025 was 100% of that year's arrears income. Both now read `rentabilidadActual`, from `utils/fundProjection.js`.
  Its **denominator** was also wrong for what the card claims. Dividing the profit by `totalAhorradoGeneral` answers "what did the money the members *hold* return"; dividing by the **capital that actually worked** answers "what did the money the fund *had to lend* return" — the return on average capital, which is what a fund reports. Money that arrived in November inflates the equity without having been able to earn anything, so the old figure systematically understated the fund (measured on the seeded fund: 1,44% → 1,62%). `GET /dashboard-stats` now returns `capitalPonderadoTotal`, computed with the **same `services/reparto.js`** the profit distribution uses, so the two screens cannot show different yields; if it fails to compute, the card falls back to equity and says so in its own caption. `pruebas_reparto_http.js` asserts the two agree to the peso.
