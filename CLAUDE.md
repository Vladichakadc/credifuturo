...
**Who sees them.** Both matrices are in `JUNTA_ROUTES`, so the board reads them from the *member* dashboard (`/dashboard/junta-matriz-ahorros`, `/dashboard/junta-matriz-cuotas`) using the same page components as the admin — no second copy to drift. Only the `GET` is opened: applying an abono, redistributing a payment or setting a policy still falls to the admin-only default gate. The savings matrix's per-cell detail calls `/savings/list`, which is *not* open to the board; on 403 it says so explicitly rather than rendering "sin movimientos", because claiming a member made no deposit when access was simply denied is the worst failure a control screen can have.
