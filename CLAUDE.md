...
- **Profit distribution (reparto de utilidades).** The fund's profit is divided in proportion to each member's **capital-time**: how much of their money the fund had available to lend, and for how many days. `services/reparto.js` holds the arithmetic (pure, no DB), `GET /savings/ranking` applies it, and `client/src/pages/shared/RepartoUtilidadesPage.jsx` renders it. The screen replaced the old "Ranking de Ahorro" and its Top-3 podium.
  - **Weight by the day the money arrived, never by the month it credits.** The previous calculation weighted each movement by `mesAbonado`, so a member who paid twelve instalments in January, one who paid month by month, and one who paid all twelve in December were credited **exactly the same** average balance — their credited months are 1–12 in all three cases. Measured on twelve $200.000 instalments: $1.300.000 recognised for all three, where the January payer actually contributed $2.307.945 and the December payer $78.904. `date` is the only field that survives both the form and the Excel import with the same meaning; see the header of `services/reparto.js` for why `year`/`monthInt` are unusable.
  - **`saldoPromedio = Σ (importe × días dentro del período) ÷ días del período`** — literally the average balance the fund held for that member. Anything dated before the period collapses into the opening balance and weighs the full period.
  - **A withdrawal is dated, not just subtracted.** Money taken out in March worked until March; netting the amount against the whole period would erase those two and a half months. It enters as a negative movement with its own value date.
  - **In the running year the cut is today, not 31 December.** Weighting against a close that has not happened would make January worth 1,0 and today 0,0 over a year still in progress, and every participation would drift daily for a reason nobody wrote down. With the cut at today the figure is the average balance *so far*, which is stable and explainable.
  - **The fact and the policy are computed separately and shown separately.** `saldoPromedio` is arithmetic and not negotiable. On top of it sit two Junta decisions, both stored in `AppSettings`, both simulable on screen before saving, and **both defaulting to the value that moves no money**: `reparto.factorPermanencia` (how much extra weight the opening balance a member did *not* withdraw carries; `1` = no premium) and `reparto.incluyeAporteInicial` (`0` = the fund's historical criterion). Shipping a default that redistributes money would be deciding it on the Junta's behalf.
  - **The permanence premium applies only to `min(saldoApertura, saldoCierre)`** — the part of the opening balance that actually stayed. A member who opened the year with $5.000.000 and withdrew it in March conserved nothing, and paying them a loyalty premium on a balance that is gone inverts the incentive.
  - **The distribution is exact by construction.** `repartir()` uses largest-remainder (Hare): whole pesos first, then the leftovers one by one to the largest fractional remainders. Rounding each share independently leaves the total a few pesos off what the Junta approved, and in a minute that gap has to be explained. `PUT /settings/:key` validates both parameters server-side — a factor arriving by API is money moving between members.
  - **Date quality is reported, never assumed.** Every movement carries `origenFecha` (`pago` / `periodo` / `sin`) and the endpoint returns the counts. A movement with no usable date is left out of the weighting rather than given an invented one, and the Junta panel shows how many fell to each level: a distribution screen that hides the quality of its inputs invites trust in a figure that may rest on guessed dates.
  - **Who sees what.** The route is in `BETA_ROUTES`, so it is gated by `requireAdminOrBetaTester` — which, note, additionally requires the `propuestas_enabled` AppSetting, a coupling inherited from the proposals rollout. Per-movement detail is returned only for the requesting member; admin and Junta get everyone's. Saving the parameters stays admin-only; the Junta simulates.
  Regression suites: `node server/pruebas_reparto.js` (54 assertions, pure arithmetic) and `node server/pruebas_reparto_http.js` (44 assertions over the real route).
...
- **`date`** (DATEONLY) — **the day the money actually reached the fund.** `POST /savings` requires it (it is the form's "Fecha Pago" and everything else is derived from it) and the Excel import maps it from the "fecha pago" column. It is the only field that means the same thing whichever created the row, and therefore the only one the profit distribution weights by.
