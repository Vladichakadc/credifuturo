...
router.post('/clients', async (req, res) => {
    try {
        ...
        const newClient = await Client.create({
            ...
            surname1,
            surname2,
            email: (email === '' || email === 'null' || email === 'undefined') ? null : email,
...
router.post('/disbursed-loans', async (req, res) => {
    ...
    try {
        ...
        // Consultamos idVm y orderId para continuidad
...
router.put('/disbursed-loans/:id', async (req, res) => {
    try {
        ...
        if (interesMensual && cuotas > 0) {
            ...
            // 1. Borrar cuotas existentes para este préstamo
            await LoanPayment.destroy({ where: { idVm: idVmActual } });
            console.log(`🗑️  Cuotas anteriores eliminadas para ${idVmActual}`);

            // 2. Obtener el siguiente consecutivo P{N} global
            ...
            console.log('✅ Tabla Estado de Préstamos regenerada: ' + scheduleRows.length + ' cuotas para ' + idVmActual);
...
router.post('/payments', async (req, res) => {
    try {
        ...
        const interesMensual = parseFloat(req.body.interesMensual || 0);
    ...
    try {
        ...
        const tables = [
            ...
            },
            {
                ...
                count: totalPayments,
                status: totalPayments > 0 ? (orphanPayments > 0 ? 'WARN' : 'OK') : 'WARN',
                message: orphanPayments > 0
                    ? `${totalPayments} registros (${orphanPayments} sin socio)`
