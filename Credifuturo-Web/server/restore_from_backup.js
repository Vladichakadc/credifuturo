    ...
    try {
        ...
        for (const row of clientData) {
            await Client.create({
                ...
                password: row['password'] || '$2a$10$ko3aa711kf2D23T1WSlqiOSzJBKODLePcHohJw/JmBHQrUGqWorvi',
                role: (String(row['Nombre']).toLowerCase().includes('administrador') || row['Customer_id'] === '1' || row['Customer_id'] === 1) ? 'admin' : 'user',
                genero: row['Genero'],
                pais: row['Pais'],
                ciudad: row['Ciudad'],
        ...
        for (const row of loanRows) {
            ...
            await DisbursedLoan.create({
                ...
                valorPrestado: row['valor prestado'], // Fixed mapping
