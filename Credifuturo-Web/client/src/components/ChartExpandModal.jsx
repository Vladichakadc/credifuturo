...
export function analyzeComparativeChart({ title, historic, current, projectedYearEnd, deviationPct, progressPct, masEsMejor = true }) {
    // `masEsMejor` decide el tono del análisis. Para casi todo (ahorro, préstamos,
    // intereses) crecer es bueno. Para mora es al revés — sin esta bandera, un
    // aumento de la mora se narraba como "ejecución eficiente y gestión sólida",
    // celebrando exactamente lo que el resto del panel marca en rojo como riesgo.
    const isPositive = masEsMejor ? current >= historic : current <= historic;
    ...
    const narrative = masEsMejor ? (() => {
        ...
        // Espejo del análisis anterior para indicadores donde crecer es una mala
        // señal (mora): los mismos umbrales de progressPct, tono invertido.
        if (progressPct >= 120) return `${title} se disparó un ${fmtPct(growth)} por encima del nivel del período de referencia — una señal de alerta de primer nivel. Un aumento de esta magnitud en cobros por mora suele responder a socios que dejaron de pagar cuotas de forma sostenida, no a atrasos puntuales. La proyección de cierre en ${fmt(projectedYearEnd)} (${fmtPct(projGrowth)} vs año anterior) confirma que la tendencia no se está corrigiendo sola. Plan de acción recomendado: (1) identificar los socios concentrando el mayor monto en mora, (2) revisar si hay un patrón común (sector, monto de crédito, fecha de desembolso), (3) activar cobro preventivo antes del vencimiento, y (4) evaluar reestructuración para socios con capacidad de pago comprometida antes de que la cartera se deteriore más.`;
        if (progressPct >= 100) return `${title} superó el nivel del año de referencia (${fmtPct(growth)}), lo que amerita atención del comité directivo. Más cobros por mora significan más socios pagando tarde — un ingreso que no es recurrente ni sostenible, y que suele anticipar deterioro de cartera si no se corrige. La proyección al cierre en ${fmt(projectedYearEnd)} (${fmtPct(projGrowth)} sobre el año anterior) sugiere que, sin intervención, el año cerrará peor que el anterior en este indicador. Recomendación: auditar la cartera vencida por socio y monto, y reforzar los recordatorios de pago antes de la fecha de vencimiento.`;
        if (progressPct >= 80) return `${title} se ubica en el ${progressPct.toFixed(0)}% del nivel de referencia — cerca del histórico pero todavía por debajo, con un margen del ${fmtPct(Math.abs(growth))} que conviene vigilar. Este nivel no es crítico, pero está cerca del umbral en el que la mora del año pasado se veía como problema. Recomendación: dar seguimiento quincenal a los socios con cuotas próximas a vencer para evitar que el indicador cruce el nivel de referencia.`;
        if (progressPct >= 60) return `${title} se mantiene en el ${progressPct.toFixed(0)}% del nivel histórico — un control razonable de la mora, con margen frente al año de referencia. Es una señal saludable, aunque vale la pena mantener los recordatorios de pago activos para sostener este nivel el resto del año.`;
        return `${title} está muy por debajo del nivel de referencia histórico (${progressPct.toFixed(0)}%) — un control de mora sólido. Los socios están pagando sus cuotas a tiempo de forma consistente. Recomendación: documentar qué está funcionando (recordatorios, seguimiento) para sostenerlo el resto del año.`;
    })();
    ...
    return {
        ...
        insights: [
            ...
            { label: 'Variación interanual', value: fmtPct(growth), icon: (growth >= 0) ? TrendingUp : TrendingDown, color: isPositive ? 'emerald' : 'red' },
