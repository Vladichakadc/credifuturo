...
import {
    ...
    ChevronRight, ArrowUpRight, Loader2, RefreshCw, X
} from 'lucide-react';
    ...
    return (
        <>
            <div className="flex items-center justify-between mb-4 shrink-0">
                ...
                <button onClick={() => navigate('/admin/initial-contributions/list')}
                    className="flex items-center gap-1 text-xs font-semibold text-brand-primary hover:text-brand-dark transition-colors bg-brand-primary/5 px-3 py-1.5 rounded-lg hover:bg-brand-primary/10">
                    Ver lista completa <ArrowUpRight className="h-3.5 w-3.5" />
                </button>
            </div>
            ...
            {pieData.length > 1 && (
                <div className="w-[140px] shrink-0 flex flex-col items-center justify-center">
                    <ResponsiveContainer width="100%" height={140}>
                        <PieChart>
                            <Pie data={pieData} cx="50%" cy="50%" innerRadius={36} outerRadius={56} paddingAngle={4} dataKey="value" animationDuration={1400} animationEasing="ease-out" stroke="none">
        ...
        <div className="w-full h-full pb-2">
            <ResponsiveContainer width="100%" height="100%">
                <AreaChart data={data} margin={{ top: 25, right: 15, left: 5, bottom: 5 }}>
                    ...
                    <Area type="monotone" dataKey="monto" name="Ahorro Mensual" stroke="#6366f1" strokeWidth={2.5} fillOpacity={1} fill="url(#areaGradAcct)" animationDuration={1600} animationEasing="ease-out"
                        dot={(props) => {
...
const AccountDetailPage = () => {
    ...
    const fetchData = useCallback(async () => {
        setLoading(true);
        try {
            const cedula = authUser.cedula || null;
            ...
            const [resSavings, resAportes, resProfile] = await Promise.all([
                ...
                authUser.id ? api.get(`/user/${authUser.id}`) : Promise.resolve({ data: null })
            ]);

            if (resProfile.data && resProfile.data.client) {
                setFullUserData(resProfile.data.client);
            }
            ...
            // Filter by socio identity (cedula or customerId)
            const socioFilter = (arr) => cedula
                ? arr.filter(s => String(s.clientCedula) === String(cedula) || String(s.clientCustomerId) === String(cedula))
                : arr;
    ...
    }, [toast, authUser.cedula, authUser.id]);
        ...
        <div className="space-y-5">
            ...
            {userStats.totalAhorradoGeneral > 0 && (
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-5 w-full">
                    {/* BARRAS */}
                    <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6 w-full flex flex-col">
